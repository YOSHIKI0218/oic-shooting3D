...
MofBool CGameApp::Update(void){
	...
	// プレイヤーの更新
	gPlayer.Update(gStage.IsHorizonMode());
	// 敵の更新
	for(int i = 0; i < ENEMY_COUNT; i++)
	{
	...
	float diff = (gStage.IsHorizonMode() ? 1.0f : 0.0f) - gCameraTransition;
	gCameraTransition += copysignf(fminf(0.02f,abs(diff)),diff);
	
	//縦スクロール時のカメラ情報 (cpos:カメラ座標, tpos:注視座標, vup:上方ベクトル)
	float posX = gPlayer.GetPosition().x * 0.4f;
	CVector3 cpos(posX,6.0f,-17.0f);
	CVector3 tpos(posX,0,-10);
	CVector3 vup(0,1,0);
	vup.RotationZ(gPlayer.GetPosition().x / FIELD_HALF_X * MOF_ToRadian(10.0f));
	
	//課題２　カメラの移動
	//補間の時間を 0.5f - 0.5f * MOF_COS(MOF_MATH_PI * gCameraTransition) として
	//カメラの座標、注視点をもとの座標から、課題の表の座標へ補間する。
...
MofBool CGameApp::Render(void){
	...
	if(gbDebug)
	{
		...
		if(gStage.IsHorizonMode())
		{
			CMatrix44 matRot;
			matRot.RotationZ(MOF_MATH_HALFPI);
			matWorld *= matRot;
		}
